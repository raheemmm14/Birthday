...
const aynoorView = document.getElementById('aynoor-view');
const sheenaView = document.getElementById('sheena-view');
const rejectView = document.getElementById('reject-view');
const errorMessage = document.getElementById('error-message');
const heartsContainer = document.getElementById('hearts-container');

// Preload sound effects to reduce latency
const successAudio = new Audio('tauhid-audio.mp3');
successAudio.preload = 'auto';

const wrongAudio = new Audio('angry-monkey.mp3');
wrongAudio.preload = 'auto';

const aynoorAudio = new Audio('yay-sound.mp3');
aynoorAudio.preload = 'auto';

const sheenaAudio = new Audio('sheena-audio.mp4');
sheenaAudio.preload = 'auto';
...
function handleLogin() {
    ...
    setTimeout(() => {
        if (name === 'tauhid' || name === 'shaik tauhid') {
            ...
            document.body.style.background = 'linear-gradient(to right, #ffb6c1, #ff69b4, #ffb6c1)';
            aynoorView.classList.add('active');
            startInfiniteHearts();
            aynoorAudio.currentTime = 0;
            aynoorAudio.play().catch(e => console.error("Audio play failed:", e));
        } else if (name === 'sheena') {
            document.body.style.background = 'linear-gradient(to right, #000428, #004e92, #000428)';
            sheenaView.classList.add('active');
            startSleepEmojis();
            sheenaAudio.currentTime = 0;
            sheenaAudio.play().catch(e => console.error("Audio play failed:", e));
        } else {
            // Scary red theme
...
function startSleepEmojis() {
    const isMobile = window.innerWidth < 768;
    const burstCount = isMobile ? 10 : 20;
    const intervalTime = isMobile ? 350 : 150;

    for (let i = 0; i < burstCount; i++) {
        setTimeout(createSleepEmoji, Math.random() * 500);
    }
    setInterval(createSleepEmoji, intervalTime);
}

function createSleepEmoji() {
    const emojis = ['💤', '😴', '🥱', '🌛', '✨'];
